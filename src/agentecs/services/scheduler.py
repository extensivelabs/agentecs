...
Usage:
    ...
    scheduler = SimpleScheduler(group_builder=build_single_group_plan)

    # Retry configuration (requires tenacity: pip install agentecs[retry])
    from agentecs.models.scheduling import RetryPolicy, SchedulerConfig
...
from agentecs.functions.result import merge_results
from agentecs.functions.scheduling import build_single_group_plan
...
from agentecs.protocols.execution import ExecutionGroupBuilder, SystemExecutor
...
class SimpleScheduler:
    ...
    Args:
        ...
        group_builder: Strategy for building execution groups from systems.
            Defaults to build_single_group_plan (all parallel, dev systems isolated).
    """
    ...
    def __init__(
        ...
        self._group_builder = group_builder or build_single_group_plan
    ...
    def build_execution_plan(self) -> ExecutionPlan:
        ...
        return self._group_builder(self._systems)
    ...
    async def _execute_group_async(self, world: SystemExecutor, group: ExecutionGroup) -> None:
    ...
    async def _execute_systems_async(
        self, world: SystemExecutor, systems: list[SystemDescriptor]
    ) -> list[SystemResult]:
    ...
    async def _execute_with_retry(
        self, world: SystemExecutor, system: SystemDescriptor
    ) -> SystemResult:
    ...
    def tick(self, world: SystemExecutor) -> None:
