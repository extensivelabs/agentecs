...
For built-in LLM client adapters:
...
The visualization tool is a separate package ([agentecs-viz](https://github.com/extensivelabs/agentecs-viz)):
...
This installs `agentecs` as a dependency automatically. See [Visualization](../viz/index.md) for usage.

**Local Development** (for contributing to agentecs-viz):
...
Equivalent to:
```bash
pip install agentecs[config,chroma,llm,retry,notebook]
```

Note: Visualization is installed separately with `pip install agentecs-viz`.
